...
export default function X1Slider() {
  var settings = {
    dots: false,
    speed: 500,
    slidesToShow: 1,
  ...
  return (
    <div className="flex flex-col gap-[calc(var(--one)*16)] overflow-hidden">
      <div className="rounded-[calc(var(--one)*16)] w-[calc(var(--one)*530)] h-[calc(var(--one)*450)]  overflow-hidden relative">
        <Slider
          {...settings}
          asNavFor={nav2}
      ...
      <div className="w-[calc(var(--one)*530)] overflow-hidden">
        ...
        >
          {ProductSliderData.x1.map((item, index) => (
            ...
            >
              <div className=" rounded-[calc(var(--one)*16)] w-[calc(var(--one)*150)] h-[calc(var(--one)*150)]">
                <Image
                  src={item.image}
                  alt={item.alt}
                  width={420}
